...
class Manager:
    ...
    def insert_data(self, values: list[tuple]):
        ...
        cursor = self.connection.cursor()
        execute_values(cursor,"INSERT INTO car_data (url, title, price_usd, odometer, username, image_url, images_count, car_number, car_vin) VALUES %s",values)
        self.connection.commit()
        cursor.close()
    ...
    def dump_data(self):
        ...
        with open(dump_filename,"w", encoding='utf-8') as file:
            ...
            insert_prefix = 'INSERT INTO car_data (%s) VALUES ' % (', '.join(column_names))
