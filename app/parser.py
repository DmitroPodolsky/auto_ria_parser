...
class WebUrls:
    ...
    def get_urls(self) -> list[str]:
        ...
        while True:
            ...
            soup = self.get_soup()
            row_urls = soup.find_all("div", class_ = "item ticket-title") # TODO add func fr that
...
async def fetch_data(session: ClientSession, url: str) -> tuple:
    ...
    try:
        async with session.get(url) as response:
            ...
            price = int(soup.find("div", class_="price_value").text.replace('$', '').replace(' ', ''))
            title = soup.find("h1", class_="head").text.strip()
            distance = int(soup.find("div", class_="base-information bold").text.strip().replace(' тис. км пробіг', '')) * 1000
            ...
            return (url, title, price, distance, username, photos[0].select_one("img")['src'], len(photos), car_number, car_vin)
    except Exception as e:
        logger.error(f"Error for {url}: {e}")
        return None
